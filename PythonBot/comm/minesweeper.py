...
class Minesweeper:
    ...
    async def minesweeper(self, ctx, *args):
        ...
        if (not self.running):
            ...
            if len(args) >= 4:
                ...
                if (0 >= self.mineamount) | (self.mineamount > (self.height * self.width)):
                    return await self.bot.send_message(ctx.message.channel,
                                                       "Wowowow, that difficulty is not something you could handle...")
            ...
            self.running = True
            return await self.bot.send_message(ctx.message.channel,
                                               "Game initialized with a " + str(self.height) + "x" + str(
                                                   self.width) + " board, glhf")
        else:
            # Running = True
            if len(args) > 0:
                if args[0] == "flag":
                    if len(args) > 2:
                        try:
                            ...
                            return await self.bot.send_message(ctx.message.channel,
                                                               "Those x or y values are not numbers...")
                        if (x <= 0) | (y <= 0) | (x >= self.height) | (y >= self.width):
                            return await self.bot.send_message(ctx.message.channel,
                                                               "Those x or y values are not on the board")
                    ...
                    try:
                        ...
                        return await self.bot.send_message(ctx.message.channel,
                                                           "Those x or y values are not even numbers")
                    if (x < 0) | (y < 0) | (x >= self.height) | (y >= self.width):
                        return await self.bot.send_message(ctx.message.channel,
                                                           "Those x or y values are not on the board")
                    await self.bot.send_message(ctx.message.channel,
                                                "You guessed: (" + str(y + 1) + "," + str(x + 1) + "):")
                    if not await self.guess(x, y):
                        await self.bot.send_message(ctx.message.channel,
                                                    "DEAD Hahaha (your score was: " + str(self.score) + "/" + str(
                                                        self.width * self.height - self.mineamount) + ")")
                        return await self.sendboard(ctx.message.channel)
        return await self.sendboard(ctx.message.channel)
        ...
        if self.board[x][y] == MINE:
            for a in range(0, self.height):
                for b in range(0, self.width):
                    self.image.paste(Image.open("./minesweeper/" + str(self.board[a][b]) + ".jpg"),
                                     (20 * b + 10, 20 * a + 10))
            await self.gameover()
            return False
