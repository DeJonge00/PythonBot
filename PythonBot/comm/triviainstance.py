    ...
    async def trivia(self, ctx, *args):
        ...
        if args[0] == "new":
            ...
            if game_mode.content == '1':
                ...
                return
            self.game_instances[ctx.message.channel] = trivia.Trivia(
            ...
            await self.game_instances[ctx.message.channel].get_params()
            # goobye fren
            try:
        ...
        if args[0] == "join":
            if ctx.message.channel in self.game_instances:
                await self.game_instances[ctx.message.channel].player_join(ctx.message.author)
            else:
