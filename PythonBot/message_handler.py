...
async def new(bot, message):
    if DogeReplaceS:
        if ((message.author.id == constants.DOGEid) & ("s" in message.content)):
            await bot.send_message(message.channel, "*" + message.content.replace("s", "ß"))
    if SpongeCapitalization:
                ...
                if (len(message.content.split(" ")) > 1) & (message.server.name.lower() == "9chat"):
                    if (message.content.lower().split(" ")[0] == "im") | (message.content.split(" ")[0] == "i'm"):
                        try: 
                            ...
                            pass
    if TrisChurPls:
        if (message.author.id in [constants.TRISTANid, constants.CHURROid]) & ("pls" in message.content.lower().split(" ")) & (message.server.id == constants.NINECHATid):
